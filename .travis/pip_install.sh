...
if [[ $TRAVIS_BRANCH == "master" ]];then
  echo "We're in master branch, will install dsbox-cleaning on master branch, too."
  pip install -e git+https://github.com/usc-isi-i2/dsbox-featurizer@master#egg=dsbox-cleaning --progress-bar off
else
  echo "We're not in master branch, will install dsbox-cleaning on devel branch."
  pip install -e git+https://github.com/usc-isi-i2/dsbox-cleaning@devel#egg=dsbox-cleaning --progress-bar off
fi
