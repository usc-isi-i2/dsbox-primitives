...
if [[ $TRAVIS_BRANCH == "master" ]];then
  ...
  echo "https://github.com/usc-isi-i2/dsbox-unit-test-datasets/tree/primitive_repo_featurizer_master"
  git checkout -b primitive_repo_featurizer_master
  rm -rf *
  mv ../output .
  git add .
  ...
  git push -f --quiet --set-upstream origin primitive_repo_featurizer_master

elif [[ $TRAVIS_BRANCH == "devel" ]];then
  echo "We're in devel branch, will push generate json files to."
  echo "https://github.com/usc-isi-i2/dsbox-unit-test-datasets/tree/primitive_repo_featurizer_devel"
  git checkout -b primitive_repo_featurizer_devel
  rm -rf *
  mv ../output .
  git add .
  git commit -a --message "auto_generated_files"
  git remote add upstream https://${GH_TOKEN}@github.com/usc-isi-i2/dsbox-unit-test-datasets.git
  git push -f --quiet --set-upstream origin primitive_repo_featurizer_devel
fi
